...
from ..pipeline import AudioPipelineConfig, AudioPreprocessingPipeline
...
def _audio_to_tuple(audio: Any) -> Optional[Tuple[np.ndarray, int]]:
    ...
    if isinstance(audio, tuple) and len(audio) == 2 and not isinstance(audio[0], int):
        wav, sr = audio
        wav = _normalize_audio(wav)
        return wav, int(sr)
...
def _quality_label(score: float) -> str:
    if score >= 80:
        return f"Excelente ({score:.1f}/100)"
    if score >= 60:
        return f"Boa ({score:.1f}/100)"
    if score >= 40:
        return f"Regular ({score:.1f}/100)"
    return f"Baixa ({score:.1f}/100)"


def _export_audio_with_ffmpeg(
    clean_audio: Tuple[np.ndarray, int], output_format: str
) -> str:
    wav, sr = clean_audio
    with tempfile.NamedTemporaryFile(
        prefix="qwen_tts_export_src_", suffix=".wav", delete=False
    ) as srcf:
        src_path = srcf.name
    with tempfile.NamedTemporaryFile(
        prefix="qwen_tts_export_", suffix=f".{output_format}", delete=False
    ) as outf:
        out_path = outf.name

    try:
        src_i16 = np.clip(np.asarray(wav, dtype=np.float32), -1.0, 1.0)
        src_i16 = (src_i16 * 32767.0).astype(np.int16)
        import soundfile as sf

        sf.write(src_path, src_i16, int(sr), subtype="PCM_16")
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(f"Export failed via ffmpeg: {stderr}")
                ...
                with gr.Tab("Clone & Generate (克隆并合成)"):
                    ...
                    with gr.Row():
                        with gr.Column(scale=2):
                            ...
                            remove_silence = gr.Checkbox(
                                label="Remover silêncio automaticamente (opcional)",
                                value=False,
                            )
                            preprocess_btn = gr.Button(
                                "Preprocess Reference (预处理参考音频)"
                            )
                            ref_audio_clean = gr.Audio(
                                label="Processed Reference Audio (处理后参考音频)",
                                type="numpy",
                                interactive=False,
                            )
                            ref_text = gr.Textbox(
                                ...
                                lines=2,
                                placeholder="Será preenchido automaticamente pela transcrição. Você pode editar antes de gerar.",
                            )
                            stt_conf = gr.Textbox(
                            ...
                            quality_est = gr.Textbox(
                                label="Estimated Audio Quality",
                                lines=1,
                                interactive=False,
                            )
                        ...
                        with gr.Column(scale=2):
                            ...
                            output_fmt = gr.Dropdown(
                                label="Output export format",
                                choices=["wav", "mp3", "ogg", "m4a"],
                                value="wav",
                                interactive=True,
                            )
                            btn = gr.Button("Generate (生成)", variant="primary")
                        ...
                        with gr.Column(scale=3):
                            audio_out = gr.Audio(
                                label="Output Audio (合成结果)", type="filepath"
                            )
                            err = gr.Textbox(label="Status (状态)", lines=6)

                    def run_preprocess(ref_aud, silence_rm: bool):
                        if ref_aud is None:
                            return (
                                None,
                                gr.update(value=""),
                                ...
                                gr.update(value=""),
                                {"processed": None},
                                "Envie um áudio de referência para iniciar o pré-processamento.",
                            )

                        stages = [
                            "[1/3] Convertendo e padronizando (mono/16k/PCM s16)...",
                            "[2/3] Limpando ruído com DeepFilterNet...",
                            "[3/3] Transcrevendo com Faster-Whisper (CUDA + VAD)...",
                        ]

                        try:
                            pipeline.config.remove_silence = bool(silence_rm)
                            result = pipeline.process(ref_aud)
                            clean_wav, clean_sr = result.clean_audio
                            conf_pct = result.transcript_confidence * 100.0
                            status = (
                                "\n".join(stages) + "\n\nPré-processamento concluído."
                            )
                            if result.denoise_warning:
                                gr.Warning(result.denoise_warning)
                                status += "\n⚠️ " + result.denoise_warning
                            state = {
                                "processed": {
                                    "audio": (clean_wav, int(clean_sr)),
                                    "path": result.clean_audio_path,
                                    "transcript": result.transcript_text,
                                    "confidence": float(result.transcript_confidence),
                                    "quality": float(result.quality_score),
                                    "denoise_backend": result.denoise_backend,
                                    "denoise_warning": result.denoise_warning,
                                }
                            }
                            return (
                                _wav_to_gradio_audio(clean_wav, clean_sr),
                                gr.update(value=result.transcript_text),
                                gr.update(value=f"{conf_pct:.2f}%"),
                                gr.update(value=_quality_label(result.quality_score)),
                                state,
                                status,
                            )
                        except Exception as e:
                            return (
                                ...
                                gr.update(),
                                {"processed": None},
                                f"Falha no pré-processamento: {type(e).__name__}: {e}",
                            )
                    ...
                    def run_voice_clone(
                        ...
                        try:
                            ...
                            processed = (
                                (state or {}).get("processed")
                                if isinstance(state, dict)
                                else None
                            )
                            if processed and "audio" in processed:
                                at = processed["audio"]
                            else:
                                if ref_aud is None:
                                    return (
                            ...
                            out_path = _export_audio_with_ffmpeg(
                                (wavs[0], sr), (out_fmt or "wav").strip().lower()
                            )
                            return (
                                out_path,
                                f"Finished. (生成完成) Export format: {(out_fmt or 'wav').strip().lower()}",
                            )
                        except Exception as e:
                            return None, f"{type(e).__name__}: {e}"
                    ...
                    preprocess_btn.click(
                        run_preprocess,
                        inputs=[ref_audio, remove_silence],
                        outputs=[
                            ref_audio_clean,
                            ref_text,
                            stt_conf,
...
"""
                    ...
                    def load_prompt_and_gen(file_obj, text: str, lang_disp: str):
                        try:
                            ...
                            return (
                                _wav_to_gradio_audio(wavs[0], sr),
                                "Finished. (生成完成)",
                            )
                        except Exception as e:
                            return None, (
